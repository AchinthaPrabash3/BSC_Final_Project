...
export const getTicketCountCncel = async (index, eventId) => {
  try {
    const results = await databases.listDocuments(
      databaseId,
      eventsCollectionId,
    ...
    const fullcount = results.documents[0].ticket_count;
    const newcount = Number(fullcount) + 1;
    fullcount[index] = String(newcount);
    updateTicketcount(eventId, fullcount);
  } catch (error) {
    console.error(error);
...
export const cancelTicket = async ({ eventId, ticketIndex, ticketId }) => {
  console.log(eventId, ticketIndex, ticketId);
  try {
    await databases.updateDocument(
      databaseId,
      ticketSailsCollectionId,
    ...
    getTicketCountCncel(ticketIndex, eventId);
  } catch (error) {
    console.error(error);
  }
};
